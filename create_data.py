...
def preprocess(file_path,file_type='excel'):
    ...
    result['text']=result['text'].apply(lambda x: ' '.join(re.split('[，：。,:.]', x)[0:]))
    ...
    result.loc[result['part'] == 'Multimedia','part'] = 'Multimedia/Speaker'
    result.loc[result['part'] == 'Speaker', 'part'] = 'Multimedia/Speaker'
    result['type'] = result['type'].apply(lambda x: str(x).replace('Water leakage', 'Leaks'))
    result['type'] = result['type'].apply(lambda x: str(x).replace('Black screen', 'Black/Blank screen'))
    result['type'] = result['type'].apply(lambda x: str(x).replace('Blank screen', 'Black/Blank screen'))
    result['type'] = result['type'].apply(lambda x: str(x).replace('Broken', 'Cracked'))

    result['len']=result['text'].apply(lambda x: len(x))
    result=result.loc[result['len']>5,:]
    result['complaint']=result['part']+' '+result['type']

    result.drop(columns=['part','type','len'], inplace=True)
    result.drop_duplicates(inplace=True)

    # split into train, dev and test
    result=result.sample(frac=1).reset_index(drop=True)
    split1=int(0.9*len(result))
    split2=int(0.93*len(result))
