# LearningRateScheduler, Knowledge Distillation Training
import re
import tensorflow as tf

def create_optimizer_basic_adam(loss,learning_rate):
...
def create_optimizer_warmup_adam(loss,init_learning_rate,num_train_steps,num_warmup_steps):
    ...
    if num_warmup_steps:
        ...
        warmup_steps_int=tf.cast(num_warmup_steps,tf.int32)
        global_steps_float=tf.cast(global_steps_int,tf.float32)
        warmup_steps_float=tf.cast(warmup_steps_int,tf.float32)

        warmup_pencent_done=global_steps_float/warmup_steps_float
        warmup_learning_rate=init_learning_rate*warmup_pencent_done
    ...
    (grads,_)=tf.clip_by_global_norm(grads,clip_norm=0.1)
    train_op=optimizer.apply_gradients(zip(grads,tvars),global_step=global_step)

    # for adam decay optimizer
